...
Sentiment Analysis within Natural Language Processing (NLP) has come a long way in a very short time. Both in terms of its state-of-the-art leaps and bounds within this past year with regard to new network architectures (e.g., [transformers](https://papers.nips.cc/paper/7181-attention-is-all-you-need.pdf), [transfer learning](https://github.com/huggingface/transformers), and along with these innovations, downstream tasks gained the most, including the ability to perform sentence classification with unparalleled performance.  

However, a sizeable problem within NLP persists despite these advances. Unlike other classification problems in and out of NLP, Sentiment Analysis requires extensive annotation and a threshold for human judgement. In this case, labelers had to use subjective judgement to express what they each felt counted as a "threat," "obscene" remark, etc. Some have tried to handle the problem by teaching machines how to annotate [ML annotation](https://arxiv.org/pdf/1903.06765.pdf). But it strikes me as strange we're teaching machines to generalize to data, also produced by learning machines. Other machine learning disciplines, of course, face the annotation problem, but it's far more prevalent in NLP (think about the decisions an annotator makes for summarization!). For example, in a Computer Vision classification task, it's a bit easier to identify / fail-to-identify something like a set of objects, cats/dogs, numerical readability or illegibility. Image IDs are correctly bound by boxes--or they're not. One or the other--classical classification. At the heart of the problem, what counts to very diverse people as everyday, commonsensical readability, and what counts as logical or reasonable, is that conventional wisdom is a very, very difficult thing to discern.

There's no place for that discussion, here, so I'll leave it alone with this: if it stands that we as humans hold worldviews--an epistemic stance on how we witness even the most mundane phenomena, doesn't it also stand that person likely will act in accordance to those differing worldviews? Does that make 'the mixing pot' a bad thing in data science? More diversity, more obligation to cede to a wider band of worldview, making it even harder to find the ever-shrinking intersection within the multi-circled venn diagram.
